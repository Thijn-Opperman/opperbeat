...
import { User, Mail, Calendar, Save, Camera, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
        ...
        <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
          ...
          {success && (
            <div className="mb-4 p-4 bg-[var(--surface)] border border-[var(--success)] rounded-[4px] animate-fade-in">
              <div className="flex items-center gap-2">
                ...
                <p className="text-[var(--success)] text-sm">Profiel succesvol bijgewerkt!</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-surface-elevated rounded-xl p-4 sm:p-6 border border-theme shadow-lg mb-4 sm:mb-6">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6 mb-6 sm:mb-8">
              ...
              <div className="text-center sm:text-left">
                <h2 className="text-lg sm:text-xl font-semibold text-primary mb-1">{profile?.name || user?.name || 'Gebruiker'}</h2>
                <p className="text-secondary text-sm">{profile?.email || user?.email || ''}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-surface border border-theme rounded-lg text-primary placeholder-muted focus:outline-none focus:border-[var(--primary)] transition-all duration-200"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2.5 bg-surface border border-theme rounded-lg text-primary placeholder-muted focus:outline-none focus:border-[var(--primary)] transition-all duration-200 resize-none"
                />
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between p-4 bg-surface rounded-lg border border-theme">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-secondary text-xs mt-1">{profile?.created_at ? formatDate(profile.created_at) : '—'}</p>
                  </div>
                </div>
              </div>
              <div className="flex items-center justify-between p-4 bg-surface rounded-lg border border-theme">
          ...
          <div className="mt-4 sm:mt-6 flex justify-end">
            <button 
              onClick={handleSave}
              disabled={saving}
              className="bg-[var(--primary)] hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed text-white font-medium px-5 sm:px-6 py-2 sm:py-2.5 rounded-lg transition-all duration-200 shadow-sm hover:shadow-md flex items-center gap-2 text-sm sm:text-base w-full sm:w-auto justify-center button-press hover-scale"
            >
              {saving ? (
                <>
